...
def set_mpi_omp_job_file(vers, test, num_process, num_thread):
    with open(f"./jobs/job_{vers}.sub", "w") as f:
        ...
        f.write(f"arguments = ../KMEANS_mpi_omp input{test}.inp 40 100 1 0.0001 output_files/{vers}/output{test}.txt comp_time/{vers}/comp_time{test}.csv {num_thread}\n\n")
        f.write("should_transfer_files = YES\n\n")
        f.write(f"transfer_input_files = ../KMEANS_{vers} ../test_files/input{test}.inp\n\n")
...
def run_test(vers, test, num_process, num_thread):
    ...
    if vers == "omp_mpi": set_mpi_omp_job_file(vers, test, num_process, num_thread)
    else: set_cuda_job_file(vers, test)
